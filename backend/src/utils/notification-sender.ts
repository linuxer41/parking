...
export class NotificationSender {
    ...
    try {
      ...
      switch (notification.channel) {
        ...
        case NotificationChannel.WHATSAPP:
          ...
          break;
        default:
          console.error(`Canal de notificación no soportado: ${notification.channel}`);
          await db.notification.markAsFailed(notification.id, `Canal no soportado: ${notification.channel}`);
          return false;
      }
      ...
      if (success) {
        ...
        await db.notification.markAsFailed(notification.id, 'Error al enviar la notificación');
        return false;
      }
    } catch (error) {
      console.error('Error al enviar notificación:', error);
      await db.notification.markAsFailed(notification.id, error.message || 'Error desconocido');
      return false;
    }
  }
  ...
  private static async sendEmail(notification: Notification): Promise<boolean> {
    try {
      ...
      console.log(`[EMAIL] Enviando notificación a ${notification.recipientId}: ${notification.title}`);
  ...
  private static async sendSMS(notification: Notification): Promise<boolean> {
    try {
      ...
      console.log(`[SMS] Enviando notificación a ${notification.recipientId}: ${notification.title}`);
  ...
  private static async sendPush(notification: Notification): Promise<boolean> {
    try {
      ...
      console.log(`[PUSH] Enviando notificación a ${notification.recipientId}: ${notification.title}`);
  ...
  private static async sendWhatsApp(notification: Notification): Promise<boolean> {
    try {
      ...
      console.log(`[WHATSAPP] Enviando notificación a ${notification.recipientId}: ${notification.title}`);
...
export async function processNotifications(): Promise<void> {
  try {
    ...
    const pendingNotifications = await db.notification.findPendingNotifications();
