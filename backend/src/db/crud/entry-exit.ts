...
export const entryExitCrud = {
   ...
   */
  async createEntryExit(data: EntryExitCreateRequest, parkingId: string, employeeId: string): Promise<EntryExit> {
    const entryExit = await withTransaction(async (client) => {
      ...
      const number = await this.generateEntryExitNumber(parkingId);
    ...
    const result = await this.getEntryExitById(entryExit.id);
    if (!result) {
   ...
   */
  async registerExit(id: string, data: ExitRequest): Promise<EntryExit> {
    ...
    return this.getEntryExitById(id)!;
  },

  /**
   * Buscar accesos por filtros
   */
  async findEntryExits(filters: {
    id?: string;
    parkingId?: string;
    employeeId?: string;
    vehicleId?: string;
   ...
   */
  async getEntryExitById(id: string): Promise<EntryExit | null> {
    const entryExits = await this.findEntryExits({ id: id });
   ...
   */
  async updateEntryExit(id: string, data: EntryExitUpdate): Promise<EntryExit | null> {
    const updates: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
   ...
   * Eliminar un acceso
   */
  async deleteEntryExit(id: string): Promise<boolean> {
    const result = await pool.query(`
   ...
   */
  async getActiveEntryExitsForSpot(spotId: string): Promise<EntryExit[]> {
    const result = await pool.query(`
   ...
   */
  async getActiveEntryExitsForVehicle(vehicleId: string): Promise<EntryExit[]> {
    const result = await pool.query(`
      SELECT * FROM t_entry_exit 
      WHERE "vehicleId" = $1 
        AND "status" = 'entered'
   ...
   */
  async getEntryExitStats(parkingId: string, startDate?: string, endDate?: string): Promise<{
    total: number;
    entered: number;
    exited: number;
    cancelled: number;
   ...
   */
  async generateEntryExitNumber(parkingId: string): Promise<number> {
    const result = await pool.query(`
