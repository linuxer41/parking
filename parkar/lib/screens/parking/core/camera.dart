...
class Camera with ChangeNotifier {
  ...
  final double _moveSpeed = 5.0;
  final double _zoomSpeed = 0.1;
  ...
  vector_math.Vector2 _applyBoundsToPosition(vector_math.Vector2 pos) {
    ...
    if (visibleWidth < _bounds!.width) {
      x = x.clamp(
        _bounds!.left + visibleWidth / 2,
        _bounds!.right - visibleWidth / 2,
      );
    } else {
      x = _bounds!.center.dx;
    }
    ...
    if (visibleHeight < _bounds!.height) {
      y = y.clamp(
        _bounds!.top + visibleHeight / 2,
        _bounds!.bottom - visibleHeight / 2,
      );
    } else {
      y = _bounds!.center.dy;
    }
  ...
  Offset worldToScreen(vector_math.Vector2 worldPos) {
    ...
    return viewportCenter +
        Offset(relativeToCamera.x * _zoom, relativeToCamera.y * _zoom);
  }
  ...
  Matrix4 get viewMatrix {
    ...
    // Traducir al centro del viewport (esto pone el origen del mundo en el centro map)
  ...
  Future<void> zoomToWithAnimation(
    double targetZoom,
