...
class GridManager {
  ...
  vector_math.Vector2 calculateSmartGuides(
    ...
    for (final otherElement in otherElements) {
      ...
      _checkVerticalGuides(
        center.x, left, right,
        otherCenter.x, otherLeft, otherRight,
        minVerticalDistance,
        (distance, position, type) {
          minVerticalDistance = distance;
          bestVerticalGuide = GuideInfo(position: position, isVertical: true, type: type);
        }
      );
      
      // Verificar guías horizontales (alineación vertical)
      _checkHorizontalGuides(
        center.y, top, bottom,
        otherCenter.y, otherTop, otherBottom,
        minHorizontalDistance,
        (distance, position, type) {
          minHorizontalDistance = distance;
          bestHorizontalGuide = GuideInfo(position: position, isVertical: false, type: type);
        }
      );
    }
  ...
  void _checkVerticalGuides(
    double centerX, double leftX, double rightX,
    double otherCenterX, double otherLeftX, double otherRightX,
    double minDistance,
    void Function(double distance, double position, GuideType type) onBetterGuide
  ) {
  ...
  void _checkHorizontalGuides(
    double centerY, double topY, double bottomY,
    double otherCenterY, double otherTopY, double otherBottomY,
    double minDistance,
    void Function(double distance, double position, GuideType type) onBetterGuide
  ) {
    // Alineación de centros
  ...
  List<GridLine> getVisibleGridLines(
    ...
    final double left = cameraPosition.x / zoom - viewportSize.width / 2 / zoom;
    final double top = cameraPosition.y / zoom - viewportSize.height / 2 / zoom;
    final double right = left + viewportSize.width / zoom;
    final double bottom = top + viewportSize.height / zoom;
