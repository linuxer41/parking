...
class ParkingState with ChangeNotifier {
  ...
  List<ParkingElement> get selectedElements => List.unmodifiable(_selectedElements);
  ...
  KeyboardShortcutsManager? get keyboardShortcutsManager => _keyboardShortcutsManager;
  ...
  vector_math.Vector2 findOptimalPosition(Size elementSize, vector_math.Vector2 initialPosition) {
  ...
  vector_math.Vector2? _findAlignedPosition(vector_math.Vector2 position, Size elementSize) {
    ...
    for (final element in nearbyElements) {
      ...
      final positions = [
        ...
        vector_math.Vector2(
          elementPos.x + (size.width * element.scale + elementSize.width) / 2 + 10,
          elementPos.y
        ),
    ...
    for (final element in nearbyElements) {
      ...
      final positions = [
        ...
        vector_math.Vector2(
          elementPos.x,
          elementPos.y + (size.height * element.scale + elementSize.height) / 2 + 10
        ),
        // Encima del elemento
        vector_math.Vector2(
          elementPos.x,
  ...
  vector_math.Vector2 _findPositionWithoutCollision(vector_math.Vector2 initialPosition, Size elementSize) {
  ...
  Future<void> centerViewOnPointWithAnimation(
    ...
    await _camera.centerOnPointWithAnimation(point, vsync, targetZoom: targetZoom);
  }
  ...
  Future<void> moveElementsWithAnimation(
    ...
    if (elements.length == 1) {
      ...
      _historyManager.moveElementAction(
        element, 
        oldPosition, 
        element.position
      );
  ...
  void undoLastAction() {
    ...
    switch (action.type) {
      ...
      case ActionType.multiMove:
        ...
        final positions = action.oldValues['positions'] as Map<String, vector_math.Vector2>;
        for (final element in action.elements) {
  ...
  void redoLastAction() {
    ...
    switch (action.type) {
      ...
      case ActionType.multiMove:
        ...
        final positions = action.newValues['positions'] as Map<String, vector_math.Vector2>;
        for (final element in action.elements) {
          if (positions.containsKey(element.id)) {
            element.position = positions[element.id]!;
          }
  ...
  void _applyProperties(ParkingElement element, Map<String, dynamic> properties) {
    // Propiedades comunes
