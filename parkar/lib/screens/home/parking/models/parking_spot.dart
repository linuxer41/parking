...
class ParkingSpot extends ParkingElement {
  ...
  void render(Canvas canvas, dynamic renderer) {
    final spotVisuals = ElementProperties.spotVisuals[type]!;
    final categoryVisuals = ElementProperties.spotCategoryVisuals[category]!;
    
    final width = spotVisuals.width;
    final height = spotVisuals.height;
    ...
    final Gradient gradient = LinearGradient(
      ...
      colors: isOccupied 
        ...
        : [
            HSLColor.fromColor(color).withLightness((HSLColor.fromColor(color).lightness + 0.2).clamp(0.0, 1.0)).toColor(),
            color,
    ...
    final borderPaint = Paint()
      ..color = isOccupied
          ...
          : HSLColor.fromColor(color).withLightness((HSLColor.fromColor(color).lightness + 0.3).clamp(0.0, 1.0)).toColor()
    ...
    if (isOccupied) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTRB(-width/2 + 8, -height/2 + 8, width/2 - 8, -height/2 + 26),
      ...
      final statusText = category == SpotCategory.normal ? "DISPONIBLE" : categoryVisuals.label.toUpperCase();
      final textSpan = TextSpan(
        text: statusText,
        style: TextStyle(
